...
export default function AdminLogin() {
    ...
    // Animation effects (from original component)
    useEffect(() => {
        ...
        try {
            ...
            if (res.ok && data.response) {
                ...
                Swal.fire({
                    icon: 'error',
                    title: 'Login Failed',
                    text: data.data || 'Invalid credentials',
                });
            }
        } catch (error) {
    ...
    const handleVerifyOtp = async (e) => {
        ...
        try {
            ...
            if (result?.error) {
                Swal.fire({ icon: 'error', title: 'Verification Failed', text: result.error });
            } else {
                router.push("/admin");
            }
        } catch (error) {
            console.error("OTP Verification Error:", error);
            Swal.fire({ icon: 'error', title: 'Error', text: 'Something went wrong during verification.' });
        } finally {
            setLoading(false);
        }
    };
    ...
    return (
        <div className="min-h-screen grid lg:grid-cols-2 bg-background font-sans">
            ...
            <div className="relative hidden lg:flex flex-col justify-between p-12 text-white overflow-hidden"
                style={{ background: "linear-gradient(135deg, #c8102e, #680014, #171516)" }}>
                ...
                <div className="relative z-20 flex items-center gap-8 text-sm text-white/60">
                    <span>&copy; {new Date().getFullYear()} Brit FinTech Awards</span>
                </div>
            ...
            <div className="flex items-center justify-center p-8 bg-background">
                <div className="w-full max-w-[420px]">
                    ...
                    {step === 1 ? (
                        <form onSubmit={handleInitLogin} className="space-y-5">
                            <div className="space-y-2">
                                <Label htmlFor="email">Email</Label>
                                <Input id="email" type="email" placeholder="admin@britfintechawards.com" value={email} onChange={(e) => setEmail(e.target.value)} onFocus={() => setIsTyping(true)} onBlur={() => setIsTyping(false)} required className="h-12 border-[#ad0b27]/20 focus-visible:ring-[#ad0b27]" />
                            </div>
                            <div className="space-y-2">
                                <Label htmlFor="password">Password</Label>
                                <div className="relative">
                                    <Input id="password" type={showPassword ? "text" : "password"} placeholder="••••••••" value={password} onChange={(e) => setPassword(e.target.value)} required className="h-12 pr-10 border-[#ad0b27]/20 focus-visible:ring-[#ad0b27]" />
                        ...
                        <form onSubmit={handleVerifyOtp} className="space-y-6 flex flex-col items-center">
                            ...
                            <button type="button" onClick={() => setStep(1)} className="w-full text-center text-sm text-[#635bff] hover:underline">
                                Back to sign in
                            </button>
                        </form>
                    )}
